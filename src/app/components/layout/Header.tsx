  ...
  return (
    <>
      <header className="h-16 bg-background border-b border-border flex items-center px-4 sm:px-6 gap-4 sticky top-0 z-30 transition-colors duration-200">
        ...
        {/* Brand Logo for Guests */}
        {!profile && (
          <div className="flex items-center gap-2 cursor-pointer shrink-0" onClick={() => navigate("/")}>
            <Logo className="w-8 h-8 text-blue-750" />
            <span className="text-foreground text-base font-bold tracking-tight">InHub</span>
          </div>
        )}
        ...
        <div className="flex items-center gap-4 ml-auto">
          {profile ? (
            <>
              ...
              {/* User Avatar */}
              <div 
                onClick={() => navigate(profile.role === 'tutor' ? '/app/tutor/settings' : '/app/settings')}
                className="w-9 h-9 rounded-full overflow-hidden flex-shrink-0 cursor-pointer ring-2 ring-blue-100 hover:ring-blue-300 transition-all"
              >
                {profile.avatar_url ? (
            ...
            <button
              ...
              className="bg-blue-700 hover:bg-blue-800 text-white px-5 py-2.5 rounded-xl text-sm font-semibold transition-all shadow-md shadow-blue-200 cursor-pointer"
            >
              Sign In
            </button>
          )}
